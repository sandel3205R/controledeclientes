              ...
              {expiringToday.length > 0 && (
                <>
                  ...
                  {expiringToday.map((client) => (
                    <NotificationItem key={client.id} client={client} onClick={() => navigate("/clients")} />
                  ))}
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            {client.plan_price ? `R$ ${client.plan_price.toFixed(2).replace(".", ",")}` : "Sem valor definido"}
          </p>
        </div>
        {client.phone && (
          <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={openWhatsApp}>
